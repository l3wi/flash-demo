...
function absorbAddressDigests(multisig, digests) {
  ...
  multisig.securitySum += digests.reduce(
    (sum, digest) => sum + digest.security,
    0
  )
  return multisig
}
...
function initTreeWithCount(flash, txCount) {
  return initializeTree(
    flash,
    Math.ceil(Math.log(txCount) / Math.log(constants.MAX_USES))
  )
}
...
function addBranch(root, start, branch) {
  ...
  for (
    node = root;
    node.address != start.address && node.children.length != 0;
...
function updateLeafToRoot(root) {
  ...
  for (i = 0; i < multisigs.length - 1; i++) {
    if (
      !multisigs[i].bundles.find(bundle =>
        bundle.find(
          tx => tx.value > 0 && tx.address == multisigs[i + 1].address
        )
      )
    ) {
      return {
  ...
  for (
    i = multisigs.length;
    i-- > 0 && multisigs[i].bundles.length == constants.MAX_USES;
...
function getMinimumBranch(root) {
  ...
  while (
    ...
    node.bundles.length == constants.MAX_USES
  ) {
    node = node.children[node.children.length - 1]
    multisigs.push(node)
  }
